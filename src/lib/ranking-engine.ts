...
function inferTaskRole(task: WorkflowTask): TaskRole {
  ...
  // Approval must be an explicit gate. The phrase "approved content" alone
  // is not an approval task and must not override a publishing task.
  const explicitApprovalPattern =
  ...
  if (explicitApprovalPattern.test(text)) return 'approval';
  ...
  if (includesAny(text, ROLE_RULES.analyze.keywords)) return 'analyze';
...
export function rankToolsForTask(
  ...
  return candidateTools
    .map((tool) => {
      ...
      // broad or imperfect.
      if (
        ['analyze', 'summarize', 'write'].includes(role) &&
