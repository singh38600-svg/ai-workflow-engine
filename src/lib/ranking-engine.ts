  ...
  return tools
    ...
    .map((tool) => {
      ...
      const isPreferredCategory = task.preferredToolCategories.some(
        (cat) =>
          tool.category.toLowerCase() === cat.toLowerCase() ||
          tool.subcategories.some(
            (sub) => sub.toLowerCase() === cat.toLowerCase()
          )
      );
      ...
      if (matchingCaps.length > 0) {
        ...
        reasons.push(
          `Supports requested capabilities: ${matchingCaps.join(', ')}`
        );
      ...
      if (previousToolSlug) {
        ...
        if (directIntegration) {
          ...
          if (toolIntegratesWithAutomation) {
            compatibilityFit += 10;
            reasons.push(
              'Integrates via automation platform (Make/Zapier/n8n).'
            );
          } else {
            compatibilityFit += 5;
      ...
      if (budgetType === 'free') {
        if (tool.free_plan_available) {
          ...
          budgetFit += 0;
          penalties.push(
            'No verified free tier (user requested free tools only).'
          );
        }
      } else {
        if (tool.free_plan_available) {
          ...
          budgetFit += 10;
          reasons.push(
            `Starting price (${tool.pricing_currency} ${tool.starting_monthly_price}/mo) is within budget.`
          );
        } else if (
          userMaxAmount !== null &&
          tool.starting_monthly_price > userMaxAmount
      ...
      if (toolDiff <= userDiff) {
        skillFit += 15;
        reasons.push(
          `Difficulty matches user's skills (${tool.technical_difficulty}).`
        );
      } else if (toolDiff - userDiff === 1) {
        skillFit += 8;
        penalties.push(
          `Slightly above comfort level (requires ${tool.technical_difficulty} setup).`
        );
      } else {
        skillFit += 2;
        penalties.push(
          `Requires higher technical expertise (${tool.technical_difficulty}) than selected.`
        );
      }
      ...
      if (
        ...
        if (tool.privacy_level === 'high') {
          privacyFit += 10;
          reasons.push(
            'Excellent enterprise-grade privacy and zero-data training logs.'
          );
          ...
          privacyFit += 1;
          penalties.push(
            'Low-privacy/public model defaults - warning for sensitive data.'
          );
        }
      } else {
        // Standard baseline
      ...
      const verificationContrib =
        tool.verification_status === 'verified' ? 3 : 1; // up to 3 pts

      reliabilityFit = Math.min(
        10,
        reliabilityContrib + verificationContrib
      );
...
export function checkCompatibility(
  ...
  const sourceAutomation = sourceTool.direct_integrations.some((slug) =>
    ['make', 'zapier', 'n8n'].includes(slug)
  );

  const targetAutomation = targetTool.direct_integrations.some((slug) =>
    ['make', 'zapier', 'n8n'].includes(slug)
  );
