...
const APPROVAL_PATTERN =
  /\b(human approval|approval gate|manual approval|approval before|approve before|review before|approve or reject|sign[- ]?off|verification gate|asks? for (?:my |human |manual )?approval|never publish without(?: human)? approval)\b/i;
...
function buildCanonicalTasks(
  ...
  if (numberedSteps.length >= 2) {
    tasks = numberedSteps.map((sentence, index) => {
...
export async function POST(request: Request) {
  try {
    ...
    if (isKeyAvailable) {
      try {
        const numberedGoalSteps = extractNumberedGoalSteps(goal);

        const call1System =
          'You are an elite AI Workflow Architect. Return only valid JSON. Preserve every explicitly numbered user step in the same order and with the same meaning. Do not replace requested tasks with different tasks.';
...
Explicit numbered step count detected by server: ${numberedGoalSteps.length}

STRICT RULES:
1. When the user supplied numbered steps, return exactly that many tasks.
  ...
  "tasks": [
    {
      ...
      "requiresHumanApproval": false
        ...
        const rawExtracted = JSON.parse(cleanedCall1);
        ...
        console.info(
          ...
          {
            numberedStepCount: numberedGoalSteps.length,
        ...
        for (const task of tasksArray) {
          ...
          workflowSteps.push({
            ...
            whySelected:
              topScored.reasons.slice(0, 2).join('. ') ||
              `Highly ranked for this ${inferTaskKind(
                `${task.title} ${task.purpose}`
              )} task.`,
...
Use this exact structure:
{
  ...
  "optimisationSuggestions": []
        ...
        // Keep simple prompts simple: when approval is requested but the
        // generated plan has no separate approval node, turn the final
        // publishing/reply step into a human-gated step instead of failing
        // the entire workflow.
        if (extracted.humanApprovalRequired) {
          const hasApprovalAction = finalSteps.some(
            workflowStepIsApproval
          );

          if (!hasApprovalAction && finalSteps.length > 0) {
            const publishingIndex = finalSteps.findIndex(
              workflowStepIsPublish
            );
            const approvalTargetIndex =
              publishingIndex >= 0
                ? publishingIndex
                : finalSteps.length - 1;

            finalSteps[approvalTargetIndex] = {
              ...finalSteps[approvalTargetIndex],
              humanAction:
                'Approve, reject, or request changes before this response is posted or published.'
            };
          }
        }
