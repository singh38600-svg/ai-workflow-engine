...
function inferTaskKind(text: string): TaskKind {
  ...
  // Sending an alert is a notification job. Check this before analysis
  // because phrases such as "alert about critical problems" also contain
  // analysis keywords like "critical problems".
  if (
    /\b(notify|notification|alert|send(?:s|ing)? (?:an )?(?:email|alert|message)|email report|message team|slack message)\b/i.test(
      low
    )
  ) {
    return 'notify';
